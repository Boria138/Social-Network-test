...
- Добавлен статический SVG favicon (`client/assets/favicon.svg`) вместо emoji data URI.

### Изменено
- Унифицирован hover-стиль для кнопок действий сообщения (`reply`, `forward`, `edit`, `delete`, `reaction`).
- Централизован рендер эмодзи через Twemoji SVG для статического и динамического контента (с `MutationObserver`).
- Символьные иконки интерфейса (`✓`, `✕`, `⛶`, `×`) заменены на inline SVG в кнопках и контролах.
